...
### Phase 5.3 — 백엔드 가격 보고서와 주문 연동 — 완료
...
### Phase 5.4 — 웹 종단간 검증 — 완료

- 견적 발급과 주문 확정을 명시적으로 분리하고 시장 가격·서명 가격·관측/만료 시각·최종 온체인 가격 표시
- 입력 변경·만료·재사용 불가 오류에 재견적 안내를 제공하고 주문·포트폴리오 WebSocket 상태 확인
- 실제 PostgreSQL에서 동일 `quoteId` 동시 소비 중 하나만 성공함을 검증하고 브라우저·백엔드·DB·Anvil 확인 절차 문서화
