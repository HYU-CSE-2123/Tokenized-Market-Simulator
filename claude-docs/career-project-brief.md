...
Ethereum ERC-20 기반 모의 원화 `mKRW`로 삼성전자 실시간 기준 가격을 추종하는 모의 토큰 `mSEC`를 매수·매도하는 학습용 하이브리드 거래소다. Spring Boot와 PostgreSQL이 인증·사용자별 내부 원장·주문·조회·실시간 API를 담당하고, 스마트 컨트랙트가 Oracle 가격에 따른 토큰 발행·소각과 거래 정산을 담당한다. Toss 증권 API의 현재가·시장 상태·과거 캔들을 수신하며, 백엔드는 온체인 트랜잭션을 비동기로 추적해 검증된 receipt만 DB 체결로 반영하고 STOMP WebSocket으로 결과를 전달한다. 브라우저 검증 클라이언트에서는 로그인부터 거래·포트폴리오·실시간 다중 주기 차트까지 MVP 흐름을 확인할 수 있다.
...
- 장 마감·휴장 또는 오래된 가격일 때 신규 주문과 Oracle 갱신 차단
...
- 백엔드 전체 자동 테스트: 120개 중 117개 통과, 선택적 Anvil 테스트 3개 기본 실행에서 skipped, 실패·오류 0개
...
- 웹 단위 테스트: 10개 통과, 실패 0개
- Vite production build: 85개 모듈 변환 성공
...
- 사용자가 확인한 화면 가격과 실제 `PriceOracle` 체결 가격의 편차 정책 정의
- 견적 생성 시각·만료 시간과 만료 후 재견적 흐름 설계
- `minTokenOut`, `minKrwOut` 또는 허용 가격 변동폭을 통한 최소 수령량 보호 검토
- 시장 상태가 바뀌거나 가격이 오래됐을 때 주문을 안전하게 거부하는 오류 계약 정리
- 모든 `FILLED` 주문과 확정 온체인 transaction의 1:1 대응을 조회 가능한 형태로 보강

### 8.2 웹 기반 MVP 완성 검증 `[계획]`

- 회원가입·로그인→faucet→견적→온체인 매수·매도→주문 상태→포트폴리오·내역을 웹에서 종단간 재검증
- 실제 Toss 현재가·시장 상태·여섯 주기 차트와 Oracle 체결 흐름을 같은 화면에서 확인
- `PENDING_ONCHAIN → FILLED/FAILED`, WebSocket 재연결과 REST 재동기화, 오류 상태를 사용자 관점에서 검증
- PostgreSQL·Anvil·Toss·Spring Boot·Vite를 포함한 재현 가능한 통합 실행 절차 정리

### 8.3 안정화와 인수인계 `[계획]`

- 현재 코드로 Solidity·백엔드·Anvil·PostgreSQL 전체 회귀 테스트
- 로컬 Anvil 기반 회원가입→faucet→실제 기준 가격 반영→온체인 매수→가격 변화→매도 시연
- 장애·재시작·중복 reconciliation과 데이터 보존 시나리오 재검증
- 아키텍처·DB·API·WebSocket·컨트랙트·실행 절차 최종 문서
- Android 담당자가 서버 구현을 몰라도 연결할 수 있는 계약과 예제 payload 제공
- 웹 기준 최종 시연 자료·화면 캡처·영상
...
- 사용자별 faucet 횟수·한도와 운영자 mKRW 준비금 정책 고도화
- DB 사용자 자산 합계와 운영자 온체인 자산을 비교하는 Proof of Reserves 또는 준비금 reconciliation

### 8.5 Android `[담당 외·최후순위]`

- Android 구현은 현재 프로젝트의 백엔드·컨트랙트 MVP 완료 조건에 포함하지 않는다.
- 모든 기능을 웹에서 먼저 동작 확인한 뒤 REST·JWT·Native STOMP 계약과 재연결 원칙을 Android 담당자에게 전달한다.
- 실제 Android UI 구현·기기 검증은 담당자 일정에 따라 별도로 진행한다.
...
> 시뮬레이션 가격에 한정됐던 거래소를 실제 삼성전자 기준 가격과 연결하기 위해 시장 데이터 공급자 경계를 설계하고 Toss REST·WebSocket을 연동했습니다. 시장 상태와 가격 신선도에 따라 주문과 Oracle 갱신을 통제했으며, 과거 1분봉을 공통 OHLCV 모델로 정규화해 5분·15분·30분·1시간 봉으로 집계했습니다. 브라우저에서는 REST 과거 봉과 실시간 체결 tick을 결합하고, 재연결이나 주기 변경 중 비동기 응답이 섞이지 않도록 요청 세대를 격리했습니다.

## 12. 자소서 작성 에이전트 지침

1. 지원 직무와 문항을 먼저 확인하고 관련 역량만 선택한다.
2. 수치가 필요하면 이 문서의 검증 수치만 사용하고 임의의 사용자 수·성능 향상률·매출을 만들지 않는다.
3. Android 앱, 거래 가격 편차 보호, 준비금 reconciliation, 사용자별 지갑, Sepolia 배포를 완료했다고 쓰지 않는다.
