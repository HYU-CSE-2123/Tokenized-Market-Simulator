...
Phase 3, Phase 4와 실제 Toss 가격 공급자, 장·가격 가용성 정책, 공급자 공통 캔들 API 및 과거 탐색이 가능한 브라우저 실시간 차트까지 구현했습니다. 다음은 화면 가격과 온체인 Oracle 체결가의 편차·만료·최소 수령량 보호를 설계하고, 모든 MVP 기능을 웹에서 종단간 검증하는 단계입니다. Android 구현은 별도 담당 범위로 최후순위에 두며, Google OAuth, 이메일 인증과 계정 연결, 리프레시 토큰은 별도 설계 승인 후 진행합니다.
