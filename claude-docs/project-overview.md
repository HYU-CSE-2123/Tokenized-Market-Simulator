...
Phase 3, Phase 4와 실제 Toss 가격 공급자, 장·가격 가용성 정책, 공급자 공통 캔들 API 및 과거 탐색이 가능한 브라우저 실시간 차트까지 구현했습니다. Phase 5.1에서 [`oracle-price-execution-design.md`](oracle-price-execution-design.md)의 주문별 서명 가격 계약과 공통 테스트 벡터를 확정했습니다. 다음은 Phase 5.2에서 `PriceOracle`의 서명 검증과 `ExchangeVault` 거래를 한 트랜잭션으로 원자화하는 단계입니다. Android 구현은 별도 담당 범위로 최후순위에 두며, Google OAuth, 이메일 인증과 계정 연결, 리프레시 토큰은 별도 설계 승인 후 진행합니다.
