...
Phase 3, Phase 4와 실제 Toss 가격 공급자, 장·가격 가용성 정책, 공급자 공통 캔들 API 및 과거 탐색이 가능한 브라우저 실시간 차트까지 구현했습니다. Phase 5.1에서 주문별 서명 가격 계약을 확정하고 Phase 5.2-A에서 `PriceOracle`의 서명·시간·replay 검증을 구현했습니다. 다음은 Phase 5.2-B에서 Vault가 같은 트랜잭션 안에서 보고서를 소비하고 그 가격으로 정산하도록 연결하는 단계입니다. Android 구현은 별도 담당 범위로 최후순위에 두며, Google OAuth, 이메일 인증과 계정 연결, 리프레시 토큰은 별도 설계 승인 후 진행합니다.
