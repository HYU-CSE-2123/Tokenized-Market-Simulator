...
- 현재 구현은 시뮬레이션 가격을 사용하며, 실제 삼성전자 시세 제공자 연동은 다음 설계·개발 과제다.
...
Phase 3, Phase 4와 브라우저 사전 검증 도구인 Phase 4.5가 완료되었습니다. 실제 삼성전자 초기 가격을 읽는 Toss REST 공급자까지 구현됐으며 개발·자동 테스트를 위한 시뮬레이션 가격 모드는 유지합니다. Android 연결 전에 Toss 실시간 WebSocket, 장 운영시간, 지연·오래된 가격 및 장애 시 거래·Oracle 정책을 세부 단계별로 설계합니다. 이후 Phase 5 Android 연결 범위를 다시 설계하며, Google OAuth, 이메일 인증과 계정 연결, 리프레시 토큰은 별도 설계 승인 후 진행합니다.
