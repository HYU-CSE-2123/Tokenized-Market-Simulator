...
Phase 3, Phase 4와 실제 Toss 가격 공급자, 장·가격 가용성 정책, 공급자 공통 캔들 API 및 브라우저 실시간 차트까지 구현했습니다. 다음은 화면 가격과 온체인 Oracle 체결가의 편차·만료·최소 수령량 보호를 설계하고, 장중 실제 Toss 전체 흐름을 인수 검증한 뒤 Phase 5 Android 연결 범위를 확정합니다. Google OAuth, 이메일 인증과 계정 연결, 리프레시 토큰은 별도 설계 승인 후 진행합니다.
