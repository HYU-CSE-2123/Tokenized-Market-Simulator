...
Phase 3, Phase 4와 실제 Toss 가격 공급자, 장·가격 가용성 정책, 공급자 공통 캔들 API 및 과거 탐색이 가능한 브라우저 실시간 차트까지 구현했습니다. Phase 5.1에서 주문별 서명 가격 계약을 확정하고 Phase 5.2-A/B에서 `PriceOracle` 검증과 `ExchangeVault` 원자적 정산을 구현했으며 Phase 5.3-A에서 백엔드 보고서 발급·서명 기반을 추가했습니다. 다음은 Phase 5.3-B에서 견적 소유권과 API 계약을 구현하고 Phase 5.3-C에서 web3j 주문 전송을 새 ABI로 전환하는 단계입니다. Android 구현은 별도 담당 범위로 최후순위에 둡니다.
