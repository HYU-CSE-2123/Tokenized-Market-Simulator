...
Phase 3, Phase 4와 브라우저 사전 검증 도구인 Phase 4.5가 완료되었습니다. Toss REST 초기 가격과 WebSocket 실시간 체결 공급자를 구현했으며 개발·자동 테스트를 위한 시뮬레이션 가격 모드는 유지합니다. Android 연결 전에 장 운영시간, 지연·오래된 가격 및 장애 시 거래·Oracle 정책을 설계합니다. 이후 Phase 5 Android 연결 범위를 다시 설계하며, Google OAuth, 이메일 인증과 계정 연결, 리프레시 토큰은 별도 설계 승인 후 진행합니다.
