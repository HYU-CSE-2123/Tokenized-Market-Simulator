...
차트는 최초 로딩·주기 변경·WebSocket 재연결 때 REST 캔들을 다시 조회해 놓친 구간을 복구합니다. 현재 봉은 이벤트의 서버 발행 시각이 아니라 공급자의 `observedAt`으로 선택한 주기의 벽시계 구간을 정하고, Toss에서는 실제 체결량을 누적하며 시뮬레이션에서는 tick 하나를 거래량 1로 표시합니다.
...
`npm test`는 캔들 정렬, 1분~1시간 구간 계산, 동일 구간 OHLCV 병합, 새 구간 생성, 오래된 tick 무시, KST 일봉 경계와 REST 로딩·주기 변경·재연결 중 WebSocket tick 경합을 검증합니다.
