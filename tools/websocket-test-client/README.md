...
8. 매수 또는 매도 후 공개 체결, 개인 주문, 개인 포트폴리오 이벤트를 확인합니다.
9. 연결을 끊고 SockJS `/ws-sockjs`를 선택해 같은 흐름을 확인합니다.
10. JWT를 지우고 연결하면 공개 이벤트만 구독되는지 확인합니다.
...
`npm test`는 캔들 정렬, 1분~1시간 구간 계산, 동일 구간 OHLCV 병합, 새 구간 생성, 오래된 tick 무시, KST 일봉 경계, 과거 페이지 병합·cursor 종료·재시도와 REST 로딩·주기 변경·재연결 중 WebSocket tick 경합을 검증합니다.
